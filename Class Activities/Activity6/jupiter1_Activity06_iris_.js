...
function split_and_matrix(data) {
    // Split the data into rows
    const rows = iris.split('\n');

    // Remove the header row
    const header = rows[0].split(',');
    rows.shift();

    // Iterate over each row and split it into values
    rows.forEach(row => {
        const values = row.split(',');
        const obj = {};

        // Map the values to the corresponding header keys
        header.forEach((key, index) => {
            obj[key] = values[index];
        });

        // Push the object to the matrix
        data.push(obj);
    });
...
function read_matrix(matrix) {
    ...
    matrix.forEach(row => {
        // Extract iris type from the last element of the row
        const iris_type = row['Class'];

        // Increment counter based on iris type
        if (iris_type === 'Iris-setosa') {
            setosa_count++;
            setosa_sepal_Length += parseFloat(row['Sepal.Length']);
            setosa_sepal_Width += parseFloat(row['Sepal.Width']);
            setosa_petal_Length += parseFloat(row['Petal.Length']);
            setosa_petal_Width += parseFloat(row['Petal.Width']);
        } else if (iris_type === 'Iris-versicolor') {
            versicolor_count++;
            versicolor_sepal_Length += parseFloat(row['Sepal.Length']);
            versicolor_sepal_Width += parseFloat(row['Sepal.Width']);
            versicolor_petal_Length += parseFloat(row['Petal.Length']);
            versicolor_petal_Width += parseFloat(row['Petal.Width']);
        } else if (iris_type === 'Iris-virginica') {
            virginica_count++;
            virginica_sepal_Length += parseFloat(row['Sepal.Length']);
            virginica_sepal_Width += parseFloat(row['Sepal.Width']);
            virginica_petal_Length += parseFloat(row['Petal.Length']);
            virginica_petal_Width += parseFloat(row['Petal.Width']);
        }
    });
...
const iristable = [
    ['iris', '#', 'Sepal_l', 'Sepal_W', 'Petal_l', 'Petal_W'],
